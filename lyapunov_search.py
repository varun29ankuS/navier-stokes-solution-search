...
def features(U):
    ...
    if GLOBAL:
        ...
        glob = [torch.log(Z), H / (2 * torch.sqrt(E * Z) + 1e-30), 0.5 * torch.log(Z / (E + 1e-30)), torch.log(pal / Z**2 + 1e-30), torch.clamp(wmag.max() / sZ, max=20.0), torch.clamp(-slope / 2, -1.0, 2.0).detach()]   # DETACHED: on a floor spectrum d log(shell)/du ~ 1e30 and the Lie derivative explodes; the value is readable, its gradient is not
