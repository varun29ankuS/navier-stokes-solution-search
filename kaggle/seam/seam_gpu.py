...
Here the window is long enough to see the seam reach that scale. Per output time, on the high-vorticity set:
    ...
    twist      strong twist: enstrophy-weighted sharp reversal at 1 and 2 cells (the searcher's penalised measure)
    anti       fraction with an antiparallel neighbour two cells away
...
REGISTERED (C20): at nu = 2e-3 the race variable reaches <= 1.5 before the clock, the strong twist peaks there and
falls by >= 20% before the clock, with max|w| growth < 3x; at nu = 0 the twist rises to the clock. KILL: twist still
rising at the clock with race < 1 (the roll-up outruns the cut at a resolved viscous scale).
...
if SCHEDULE is None and os.path.isdir("/kaggle/working"):
    SCHEDULE = "IC=found NU=2e-3;IC=found NU=0;IC=pair NU=2e-3;IC=found NU=1e-2"
if SCHEDULE:
...
def diag(U):
    ...
    high = wm > 0.5 * wm.max()
    anti = torch.zeros_like(high); soft = 0.0
    for ax in range(3):
        for sh in (1, 2):
            ...
            if sh == 2: anti |= (beta > 1) | (torch.roll(beta, -2, dims=ax) > 1)
    soft = (soft / (w2**2).mean()).item()
    ...
    return (0.5 * w2.mean().item(), wm.max().item(), soft, anti[high].float().mean().item(), ell, ell_nu, cut, strip(U))
...
if IC == "pair":
    ...
    D = float(os.environ.get("D", 1.2)); SIG = float(os.environ.get("SIG", 0.35)); A = float(os.environ.get("A", 0.25))
...
print("   t     Z/Z0    max|w|    twist(strong)   anti    ell     ell_nu    race=ell/ell_nu   cut     delta")
t, mark, t0 = 0.0, 0.0, time.time(); hist = []
while t <= T + 1e-9:
    if t >= mark - 1e-9:
        Z, wmax, soft, anti, ell, ell_nu, cut, d = diag(U); hist.append((t, Z / Z0, wmax, soft, anti, ell, ell_nu, cut, d))
        print("%5.2f   %6.3f   %7.2f    %.5f        %.3f   %.4f   %.4f     %6.2f          %.3f   %.4f%s   (%.0fs)" % (
            t, Z / Z0, wmax, soft, anti, ell, ell_nu, ell / ell_nu if NU > 0 else float("nan"), cut, d, "" if d > 2 * dx else "  <-- past the clock", time.time() - t0), flush=True)
        mark += EVERY
        if t >= T - 1e-9: break
    with torch.no_grad():
        ...
        U = step(U, dt); t += dt
H = np.array(hist); tt, tw, race, d = H[:, 0], H[:, 3], H[:, 5] / np.where(H[:, 6] > 0, H[:, 6], np.nan), H[:, 8]
